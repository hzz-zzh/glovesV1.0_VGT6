...
- `2026-09-17 / V3.1.0 → V3.1.1`：采样降为125Hz，128帧Storage队列的标称缓冲时长更新为1024ms，队列和内存池容量不变。
...
当前FullFrame采用136块静态内存池。Storage队列深度为128，在125Hz采样、队列初始为空时
可缓冲约1024ms的数据；实际可吸收的SD写入阻塞时长还受已有积压和任务调度影响。RS485只保留4帧队列，避免通信快照占用大量数据块。
Storage订阅默认关闭，只有日志文件创建成功后才开启；停止录制时先关闭订阅，再处理
队列中已经接收的帧，因此文件准备期间不会因为Storage队列积压产生记录内丢帧。
