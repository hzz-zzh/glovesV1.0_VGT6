...
说明：`0x1140`、`0x1340`、`0x2080` 三个数据区时间戳均表示同一帧数据的同步采集时刻 UTC，因此三个值应保持一致。关节区时间戳不是解算完成时刻，而是该帧输入数据的采集时刻。系统从未收到UTC基准前时间戳为 `0.000000000`；已有基准但当前失步时可保留估算值，主站必须在UTC有效位为0时把它视为未同步时间。
...
没有PPS时写入UTC只提供软件估算时间，不构成PPS对时。已经建立的UTC估算值可以保留，但不能仅凭非零时间戳判断有效。
...
主站应先检查snapshot status bit0和bit2，再检查IMU、关节和触觉状态。UTC未同步时bit1为0，但传感器数据仍可有效；此时可使用`frame_id`和主站本地时间处理数据。首次对时成功后的连续正常PPS期间bit1不因等待UTC刷新而清零；首次/恢复对时前采集的帧即使晚于UTC写入才被读取，bit1也不会被误置为1。PPS丢失或帧龄超过30ms时bit0清零，载荷仍保留最后一帧，仅供显示和诊断。
