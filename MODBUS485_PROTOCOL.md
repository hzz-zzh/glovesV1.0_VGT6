...
|---|---|---|---|---:|---|
| `2.1` | `2026-09-15` | `V3.1.0` | `0x0201 / 0x0201` | `3` | PPS同步采样、失效状态和高速快照协议升级 |

修订`2.1`的主要变化：

- PA15由GPIO外部中断改为TIM2 ETR硬件触发；每个PPS启动一个包含200个采样点的窗口，窗口结束后停表等待下一次PPS。
- 增加PPS首次等待、采样中、等待下一PPS和PPS丢失状态；PPS丢失后停止采样，但RS485和看门狗继续运行。
...
- 传感器失效后保留最后一帧数值供诊断，但清除数据有效位；主站必须依据有效位和帧龄决定是否使用数据。
- TIM5改为自由运行时间基准；PPS丢失或恢复但尚未重新下发UTC时，时间值可保留估算结果，但UTC有效状态必须为0。
...
遗留整块预分配空白区。Storage消费者队列可暂存128帧，对应200Hz下640ms的写卡延迟缓冲；RS485队列仍保持
4帧，FullFrame池共136块并保留发布过程余量。200Hz下持续数据率为
`1536 × 200 = 307200 byte/s`，约`1.106 GB/hour`（十进制）或`1.03 GiB/hour`；
该带宽远低于正常SD卡顺序写能力，实际风险主要来自卡内部整理造成的单次写延迟尖峰。
量产测试应观察`0x0096`和`0x0098..0x0099`：4帧批写预算为20ms；单次写入达到
640ms时仍可能耗尽128帧队列，即使平均写入速度足够也应视为不合格卡型。
...
主站应先检查snapshot status bit0和bit2，再检查IMU、关节和触觉状态。UTC未同步时bit1为0，但传感器数据仍可有效；此时可使用`frame_id`和主站本地时间处理数据。PPS丢失或帧龄超过30ms时bit0清零，载荷仍保留最后一帧，仅供显示和诊断。

当前6 Mbps、8N1链路发送一帧912字节响应约需1.52 ms，加上200 us响应等待、
请求接收、快照构造和收发方向切换后，为5 ms周期的200 Hz连续轮询保留更多余量。
当前上位机工具配合6 Mbps链路采用5 ms周期的200 Hz轮询；手套内部传感器和
FullFrame在每个PPS周期内产生200帧，每次RS485响应返回当时最新的一帧。PPS上升沿
作为本秒第0帧，后续第1至199帧由TIM2产生；第199帧同步脉冲结束后TIM2停止，下一次
...
- 使用5 ms时基调度；事务超过当前周期时记录overrun，并立即从当前时刻重建节拍；
- Windows采集期间启用1 ms高精度定时器，并提高通信线程优先级；周期末尾2 ms采用忙等，降低线程晚唤醒；
- 只在相邻事务间隔不足时补足100 us静默时间，正常5 ms节拍不再额外忙等；
- 固件版本、设备信息和健康状态在高速轮询启动前读取并缓存，运行期间只发送`0x41`，避免标准寄存器事务打断200 Hz节拍；
