...
| 地址 | 数量 | 类型 | 含义 |
|---:|---:|---|---|
| `0x000E` | 1 | U16 | 固件主版本，当前为 `2` |
| `0x000F` | 1 | U16 | 固件次版本，当前为 `2` |
...
当前6 Mbps、8N1链路发送一帧910字节响应约需1.52 ms，加上200 us响应等待、
...
- `0x41`请求写入串口驱动后，响应容错窗口为8 ms；正常收到完整910字节后立即返回，不固定等待8 ms；
...
- 固件版本和健康状态在高速轮询启动前读取并缓存，运行期间只发送`0x41`，避免标准寄存器事务打断200 Hz节拍；
