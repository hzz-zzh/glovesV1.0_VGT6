...
当前硬件由外部电源直接供电。历史硬件资料归档在 [旧版硬件说明](docs/legacy/POWER_MANAGEMENT_USER_GUIDE.md)，不适用于当前硬件和Modbus 2.2协议。

RS485寄存器、统一健康状态和历史错误清除命令见 [Modbus485协议](MODBUS485_PROTOCOL.md)。

固件版本统一定义在 `App/inc/app_version.h`，当前版本为 `V3.1.2`。

当前发布配置：125Hz PPS同步采样、标称8ms间隔，RS485采用4000000 bit/s、8N1；Modbus/Health协议版本分别为`0x0202 / 0x0201`，传感器快照Schema为`3`。首次对时及PPS失步恢复后需写入对应UTC整秒，连续正常PPS期间无需每秒写UTC。

本文档说明当前 FreeRTOS 工程中的任务划分、任务之间的数据流关系，以及推荐的任务优先级设置。

## 文档更新日志

- `2026-09-17 / V3.1.1 → V3.1.2`：增加命令启动的125Hz独立调试采集和10秒会话，上位机提供启动/停止及自动续期；正式PPS模式不变，调试UTC无效且SD禁录。Modbus升至2.2，Health及Schema不变，协议文档修订由2.2升至2.3。
...
连接4Mbps RS485后，使用`tools/modbus485_monitor.py`的“启动独立采集（无PPS）”，不接头部/PPS源也可测试IMU、触觉、组帧、算法和485链路。工具125Hz轮询，每3秒续期；“停止独立采集”或轮询Stop退出，掉线后设备最多10秒开始退出。

调试不使用模拟数据，也不能验证PPS对齐、UTC或SD录制。调试绝对时间字段为0，界面标为未同步；真实PPS不会接管。退出后正常模式等待新PPS并重新写UTC，接口限制见两份协议的“独立调试采集接口”。

无硬件接口回归：`python -B -m unittest discover -s tools -p test_standalone_debug.py -v`。这些测试覆盖监控工具的有效性、命令、续期与退出，不替代Keil编译和实板验证。

实板验收重点：

- 不接PPS，启动调试后检查16路IMU、68路触觉及关节值随动作变化，帧号递增、帧龄正常；新鲜完整数据的快照状态通常为`0x0019`，UTC字段为0。
- 调试中接入/断开PPS，确认PPS存在位随真实输入变化，但采样不重置、UTC仍无效；正常PPS存在时对应状态通常为`0x001D`。
- 调试中尝试UTC写入和SD开始，应被拒绝；SD准备、录制、停止期间申请调试，也应被拒绝。
- 点击停止或关闭工具，检查采样停止且PWM回到低电平；模拟工具异常退出、不再续期，确认约10秒后自动开始退出。
- 在PPS高电平期间退出调试，不能把GPIO切换产生的电平变化视为新PPS；输入恢复低电平后，等待后续真实上升沿才采样。PPS一直为高时应保持等待，仍允许重新启动独立调试。
- 停止后重新提供PPS，检查新采样恢复但UTC无效；写入该PPS对应UTC后，新帧才恢复UTC有效，不能混入调试旧帧或旧滤波历史。
