...
说明：正常模式的`0x1140`、`0x1340`、`0x2080`三个数据区时间戳均表示同一帧的采集时刻UTC，三个值应保持一致。关节时间戳表示输入数据的采集时刻。正常模式从未收到UTC基准前时间戳为`0.000000000`；已有基准但失步时可保留估算值，UTC有效位为0时只能视为未同步时间。独立调试及模式切换期间这三个时间戳固定为0。
...
正常模式无PPS时写UTC只建立软件估算时间；独立调试及模式切换期间拒绝UTC写入。传感器数值有效性独立于时间有效性，允许`DATA_VALID=1、UTC_VALID=0`。
...
正常模式检查snapshot status bit0和bit2及各传感器状态；独立调试（bit4=1）不要求bit2，但仍需检查数据有效及各传感器状态。首次对时成功后的连续正常PPS期间，bit1不因等待UTC刷新而清零；首次/恢复对时前采集的旧帧不会被之后的UTC写入误标为有效。UTC无效不代表传感器数值无效。正常模式PPS丢失，或任一模式帧龄超过30ms时，bit0清零；载荷保留最后一帧供诊断，不能继续用于控制。独立调试的PPS消失不会单独使数据失效。
