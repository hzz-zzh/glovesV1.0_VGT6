...
/* 诊断通过阈值为标称采样率的90%，单位为0.1Hz，125Hz对应112.5Hz。 */
#define UART_DEBUG_MIN_SAMPLE_RATE_X10 (GLOVE_SENSOR_SAMPLE_RATE_HZ * 9U)
...
static void UartDebugTask_PrintChainStatus(uint32_t sample_count)
{
    ...
    printf("[IMU_CAN] irq=%lu rx=%lu parsed=%lu unparsed=%lu rejected=%lu pub=%lu drop=%lu init_err=%lu err=%lu last=0x%08lx ext=%lu dlc=%lu cfg_tx=%lu cfg_reply=%lu cfg_ok=0x%04lx cfg_fail=0x%04lx cfg_retry=%lu rec_state=%lu rec_node=%lu bus_reinit=%lu fresh=0x%04x first_node=%lu seen=0x%08lx\r\n",
